...
int SymbolTable::insert(string name, symbolType type) {
	...
	if (DEBUG_SYMBOLTABLE) {
		...
		cout << "       Name: " << name << ", Offset: " << offset << ", Type: " << type << endl;
	}
		...
		if (mlevel->second.find(name) != mlevel->second.end()) {
			if (mlevel->second[name].type != type) {
				...
				if (DEBUG_SYMBOLTABLE) {
					...
					cout << "       Expected Type: " << type << ", Result: " << mlevel->second[name].type << endl;
				}
			...
			if (DEBUG_SYMBOLTABLE) {
				cout << "Debug: SymbolTable.lookup()" << endl;
				cout << "       Found: " << name << ", Level: " << level << ", Type: " << type << " Offset: " << offset << endl;
			}
...
void SymbolTable::print() {
	...
	for (level = content.begin(); level != content.end(); level++) {
		...
		for (pos = level->second.begin(); pos != level->second.end(); pos++) {
			...
			cout << "                   Offset: " << pos->second.offset << " Type: " << pos->second.type << endl;
		}
	}
}
